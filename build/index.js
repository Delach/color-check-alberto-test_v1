...
function rgbToOklch(r, g, b) {
    ...
    // RGB lineal
    [rs, gs, bs] = [rs, gs, bs].map((c) => c <= 0.04045 ? c / 12.92 : Math.pow((c + 0.055) / 1.055, 2.4));
    // XYZ
...
function suggestAccessibleColor(originalHex, targetRatio = 7.0, bgHex = "#FFFFFF") {
    ...
    for (let brightness = 0; brightness <= 255; brightness += 5) {
        const adjusted = rgbToHex(Math.max(0, original.r - brightness), Math.max(0, original.g - brightness), Math.max(0, original.b - brightness));
...
app.post("/api/analyze-colors", express.json({ limit: "50mb" }), (req, res) => {
    try {
...
app.post("/api/analyze-website", express.json(), async (req, res) => {
    try {
        const { url } = req.body;
        if (!url) {
        ...
        // Analyze the colors
        const analysis = topColors.map((color) => {
            const ratio = calculateContrast(color, "#FFFFFF");
            const corrected = suggestAccessibleColor(color, 7.0, "#FFFFFF");
            const correctedRgb = hexToRgb(corrected);
            return {
...
app.post("/api/simulate-deficiency", express.json(), (req, res) => {
    try {
...
app.get('/api/example-colors', (req, res) => {
